...
class FactorySkillContractTest(unittest.TestCase):
    ...
    def test_active_seedance_submission_contract_uses_runninghub_standard_model(self):
        ...
        documents = {
            ...
            "workspace environment example": ROOT / ".env.example",
    ...
    def test_bundle_includes_the_ui_interaction_contract_runtime_module(self):
        manifest = json.loads(
            (ROOT / "references" / "bundle_manifest.json").read_text(encoding="utf-8")
        )
        paths = {str(item.get("path") or "") for item in manifest.get("runtime_files", [])}
        self.assertIn("server/ui_interaction_contract.py", paths)
        self.assertIn("server/ui_interaction_contract.py", REQUIRED_SERVER_FILES)
    ...
    def test_ui_only_frame_locked_rebuild_contract_is_documented(self):
        factory = (ROOT / "SKILL.md").read_text(encoding="utf-8")
        bundled = (
            ROOT
            / "bundled-skills"
        ...
        combined = "\n".join((factory, bundled))

        for required in (
            "UI-only frame-locked rebuild",
            "opaque_ui_demo always wins",
            "source-ui-interaction/v1",
            "ui_roi_only",
            "source_frame_locked",
            "basic-anchor-only",
            "no automatic retry",
            "new_product_image",
            "new_model_image",
            "source has no UI Cut",
            "never enter Seedance",
        ):
            with self.subTest(required=required):
                self.assertIn(required, combined)
