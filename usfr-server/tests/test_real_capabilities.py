...
class _Context:
    def __init__(
        ...
        *,
        ...
        self.input_slots = (
            ...
            {
                "slot_id": "new_product_image",
                "present": product is not None,
                "values": [str(product)] if product is not None else [],
                "metadata": [{}] if product is not None else [],
            },
            {
                "slot_id": "new_model_image",
                "present": model is not None,
                "values": [str(model)] if model is not None else [],
                "metadata": [{}] if model is not None else [],
            },
            {
    ...
    def materialize_slot(self, slot_id: str, *, index: int = 0):
        if slot_id == "source_video":
            ...
            path = self.ui
        elif slot_id == "new_product_image" and self.product is not None:
            path = self.product
        elif slot_id == "new_model_image" and self.model is not None:
            path = self.model
        else:
...
class RealCapabilitySmokeTest(unittest.TestCase):
    ...
    def test_active_generated_ui_uses_product_evidence_and_forwards_frame_locked_interaction_contract(self) -> None:
        ui = self._ui_fixture()
        seen = {}
        interaction = {
            "schema_version": "source-ui-interaction/v1",
            "region_id": "ui-1",
            "source_window_us": {"start": 0, "end_exclusive": 400_000},
            "frame_window": {"start": 0, "end_exclusive": 10},
            "display_viewport": [200, 100],
            "language": {"source": "en", "target": "ar", "mode": "localized"},
            ...
            "validation": {"mode": "basic_anchor_only", "automatic_retry": False, "anchor_frames": [0, 9]},
        }
        ...
        class _VideoRenderer:
            ...
            def __call__(self, source, output, _context, *, truth, render_contract):
                seen["source"] = source
                seen["truth"] = truth
                seen["render_contract"] = render_contract
                result = subprocess.run(
                    [
                        "ffmpeg", "-y", "-loglevel", "error", "-loop", "1", "-i", str(source),
                        "-t", "0.4", "-vf", "fps=10,format=yuv420p", "-c:v", "libx264",
        ...
        self.assertEqual(seen["source"], ui)
        self.assertEqual(seen["render_contract"]["source_ui_interaction_contract"], interaction)
        self.assertEqual(seen["render_contract"]["language"], interaction["language"])
        self.assertEqual(result["ui_render_contract"]["source_ui_interaction_contract"], interaction)

    def test_active_generated_ui_rejects_malformed_frame_locked_interaction_contract_before_rendering(self) -> None:
        context = _Context(
            self.video,
            product=self._ui_fixture(),
            profile_snapshot={"profile": "high_fidelity_hybrid_v1", "activation_mode": "active"},
            timeline_regions=(
                {
                    "region_id": "ui-1",
                    "region_type": "generated_ui_demo",
                    "source_ui_interaction_contract": {
                        "schema_version": "source-ui-interaction/v1",
                        "region_id": "ui-1",
                    },
                },
        ...
        with self.assertRaisesRegex(CapabilityUnavailable, "source UI interaction contract"):
            DeterministicUiRenderer(
                ocr_backend=_EvidenceBoundOcrBackend(lambda _path: self._ocr_records()),
                production=True,
                sha256="b" * 64,
    ...
    def test_ui_text_guard_rejects_question_mark_placeholder_runs(self) -> None:
        self.assertTrue(DeterministicUiRenderer._contains_garbled_text("????"))
        self.assertFalse(DeterministicUiRenderer._contains_garbled_text("Where?"))
