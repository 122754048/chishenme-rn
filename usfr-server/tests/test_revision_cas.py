...
def script_approval(revision: int = 1, sha: str = "b" * 64, timeline_sha: str = "a" * 64):
    ...
    visible_text_locks = []
    visible_text_locks_sha256 = __import__("hashlib").sha256(
        json.dumps(visible_text_locks, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
    ).hexdigest()
    return {
...
def test_script_approval_cas_rejects_a_changed_visible_text_lock_digest():
    store, _ = make_store()
    job = create_job(store)
    appended = store.append_revision(
        job_id=job.job_id,
    ...
    approval["visible_text_locks_sha256"] = "f" * 64

    with pytest.raises(ReplicationError, match="visible_text_locks_sha256"):
        store.approve_revision(
            job_id=job.job_id,
            kind="script",
            revision=1,
...
def test_script_approval_cas_rejects_a_timeline_bound_sidecar_missing_visible_text_locks():
    store, _ = make_store()
    job = create_job(store)
    appended = store.append_revision(
        job_id=job.job_id,
    ...
    approval = script_approval()
    approval.pop("visible_text_locks")
    approval.pop("visible_text_locks_sha256")

    with pytest.raises(ReplicationError, match="visible text locks"):
        store.approve_revision(
            job_id=job.job_id,
            kind="script",
            revision=1,
