...
Before every storyboard call, load and compile the exact bytes of `references/daohuo_storyboard_prompt.md`; it is the only director-board prompt and layout authority. Bind its byte SHA-256 into the provider prompt, layout receipt, storyboard metadata, and approval artifact. RunningHub Image2 generates the complete `usfr-cinematic-director-production-board/v1` required by that template: creative header; character, face/hair and wardrobe/style evidence; ordered Cut cards; target evidence or `none`; top-down camera/movement diagram; and bottom lighting, camera, palette, audio/tone, mood and cinematography notes. Preserve the returned Image2 PNG as the approval artifact; deterministic code may validate, bind receipts, extract a distinct execution carrier, or add approved typography, but may not replace the board with a remembered layout. The obsolete five-region information board is invalid. Missing template bytes/SHA, unresolved placeholders, missing regions, missing receipt, or any mismatch fails closed.
