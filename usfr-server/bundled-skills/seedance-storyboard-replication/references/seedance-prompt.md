...
the labels-free `seedance_visual_carrier` cryptographically bound to the approved board's `storyboard_grid` ROI supply the fixed Seedance references.

The Seedance carrier contains only approved Cut visuals, not the director-board headers or notes. Scene-surface text must be repeated exactly in the matching Seedance Cut prompt with carrier ID, surface relation, placement, and motion/deformation behavior; it moves, bends, folds, rotates, occludes, and tears with its carrier. Subtitles, captions, CTA, lower-thirds, stickers, and UI text must not be generated by Seedance. 不要依赖故事板图片中的文字，也不要要求 Seedance 自行识别图片里的完整脚本、口播或备注。Repeat the complete approved script as prompt text.

Use the fixed B route for every source-fidelity generated segment: upload exactly
one matching frozen original source segment at `videoUrls[0]`, the approved-board-bound
`seedance_visual_carrier` at `imageUrls[0]` / `@Image1`, then only fixed-slot target
...
Reference order: matching original source segment at `videoUrls[0]`; approved-board-bound `seedance_visual_carrier` at `imageUrls[0]` / `@Image1`; then only fixed-slot target references. Source Cut/keyframe sheets, replacement-control sheets, approval boards, and layout receipts must never be sent to Seedance.
...
1. Actual image-number mapping. Upload only approved storyboard and target
   ...
   the approved-board-bound `seedance_visual_carrier` at `imageUrls[0]` / `@Image1`, then only
   ...
   - `@图片1` is the labels-free `seedance_visual_carrier` for the current segment; its receipt binds the exact approved director-board SHA, layout receipt, `storyboard_grid` ROI, Cut IDs, and carrier SHA.
...
`usfr-video-reference/v1`, the approved-board-bound `seedance_visual_carrier`
at `imageUrls[0]` / `@Image1`, and only fixed-slot target references afterward.
The approved director board, source Cut/keyframe sheets, replacement-control
sheets, and storyboard layout receipt must never be sent to Seedance. The request
