...
original source segment at `videoUrls[0]`, the approved-board-bound
`seedance_visual_carrier` at `imageUrls[0]` / `@Image1`, then only fixed-slot
target references; it may also carry one eligible current-segment audio
fragment. The approved director board, source Cut/keyframe sheets,
replacement-control sheets, and storyboard layout receipt must never be sent
to Seedance, and the full
