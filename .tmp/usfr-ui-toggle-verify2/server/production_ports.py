...
from .runninghub_workflows import _upload_binary
...
class VerifiedVideoReferenceRequest(dict[str, Any]):
    """Canonical standard payload plus the server-only source-slice binding.

    The binding intentionally is not serialized into the RunningHub request
    body.  It remains available at the final paid-call boundary so a bare
    mapping cannot smuggle an arbitrary video URL into ``videoUrls``.
    """

    def __init__(self, payload: Mapping[str, Any], *, video_reference_binding: Mapping[str, Any]) -> None:
        super().__init__(dict(payload))
        self.video_reference_binding = dict(video_reference_binding)
...
class ProductionEnvironment:
    ...
    def from_environ(cls, environ: Mapping[str, str] | None = None) -> "ProductionEnvironment":
        ...
        _require_secret(source, "USFR_CAPABILITY_SECRET")
        ...
        return cls(
            ...
            runninghub_seedance_upload_url=_require_https_url(source, "RUNNINGHUB_SEEDANCE_UPLOAD_URL"),
...
class EvidenceBoundGptPlanner:
    ...
    def _source_cut_evidence(
        ...
        route_output = stage_outputs.get("route_regions") if isinstance(stage_outputs, Mapping) else None
        source_bundle = route_output.get("source_evidence_bundle") if isinstance(route_output, Mapping) else None
        if isinstance(source_bundle, Mapping):
            from .source_evidence_bundle import validate_source_evidence_bundle

            bundle = validate_source_evidence_bundle(source_bundle)
            semantic_evidence = bundle.get("semantic_evidence")
            if not isinstance(semantic_evidence, Mapping):
                raise ProductionPortsError("current job source evidence bundle has no semantic evidence")
            return cls._sha256_field(bundle.get("bundle_sha256"), "source evidence bundle SHA"), cls._project_source_cuts(semantic_evidence)
    ...
    def _revision_schema(cls, kind: str) -> dict[str, Any]:
        ...
        common = {
            ...
            "visible_text_locks_sha256": string,
        }
        if kind == "script":
    ...
    @classmethod
    def _default_visible_text_locks(
        cls, source_content_timeline: Mapping[str, Any] | None
    ) -> tuple[list[dict[str, Any]], str]:
        """Turn immutable observations into the editable script's keep defaults."""

        if source_content_timeline is None:
            empty: list[dict[str, Any]] = []
            return empty, visible_text_locks_sha256(empty)
        rows = source_content_timeline.get("visible_text")
        if not isinstance(rows, list):
            raise ProductionPortsError("source content timeline visible text is invalid")
        proposed: list[dict[str, Any]] = []
        for index, row in enumerate(rows, start=1):
            if not isinstance(row, Mapping):
                raise ProductionPortsError(f"source content timeline visible text {index} is invalid")
            proposed.append(
                {
                    ...
                    "kind": row.get("kind"),
                    "source_evidence_sha256": row.get("source_evidence_sha256", row.get("evidence_sha256")),
                    "approved_text": row.get("text"),
                    "disposition": "keep",
                    "placement": row.get("placement"),
                }
            )
        try:
            locks = validate_visible_text_locks(proposed, timeline=source_content_timeline)
        except VisibleTextContractError as exc:
            raise ProductionPortsError("source content timeline visible text locks are invalid") from exc
        return locks, visible_text_locks_sha256(locks)

    @classmethod
    def _approved_visible_text_locks(
        cls,
        context: Any,
        *,
        source_content_timeline: Mapping[str, Any] | None,
        script_sha256: str,
    ) -> tuple[list[dict[str, Any]], str]:
        """Load the user-confirmed text sidecar for a storyboard draft.

        The source timeline is consulted only to validate the already-approved
        locks.  It is never placed into storyboard GPT evidence, so the draft
        can neither inspect raw text again nor replace a user confirmation.
        """

        default_locks, empty_or_default_sha = cls._default_visible_text_locks(source_content_timeline)
        if not default_locks:
            return [], empty_or_default_sha
        snapshot = getattr(context, "snapshot", None)
        revision = getattr(snapshot, "current_script_revision", None)
        if isinstance(revision, bool) or not isinstance(revision, int) or revision < 1:
            raise ProductionPortsError("storyboard requires the approved script revision for visible text")
        store = getattr(context, "job_store", None)
        get_current = getattr(store, "get_current_revision", None)
        get_approval = getattr(store, "get_script_approval", None)
        if not callable(get_current) or not callable(get_approval):
            raise ProductionPortsError("storyboard requires approved visible text locks")
        try:
            manifest = get_current(context.job_id, "script")
            approval = get_approval(context.job_id, revision)
        except Exception as exc:
            raise ProductionPortsError("storyboard could not read approved visible text locks") from exc
        if not isinstance(manifest, RevisionManifest):
            raise ProductionPortsError("storyboard approved script revision is unavailable")
        if manifest.kind != "script" or manifest.revision != revision or manifest.sha256 != script_sha256:
            raise ProductionPortsError("storyboard approved script revision does not match visible text locks")
        if not isinstance(approval, Mapping):
            raise ProductionPortsError("storyboard requires approved visible text locks")
        if approval.get("revision") != revision or str(approval.get("script_sha256") or "").lower() != script_sha256:
            raise ProductionPortsError("storyboard approved visible text locks do not match the approved script")
        if source_content_timeline is None:
            raise ProductionPortsError("storyboard source content timeline is unavailable")
        timeline_sha256 = cls._sha256_field(
            source_content_timeline.get("artifact_sha256"),
            "current job source content timeline SHA",
        )
        if str(approval.get("source_content_timeline_sha256") or "").lower() != timeline_sha256:
            raise ProductionPortsError("storyboard approved visible text locks do not match the source content timeline")
        raw_locks = approval.get("visible_text_locks")
        if not isinstance(raw_locks, list):
            raise ProductionPortsError("storyboard approved visible text locks are invalid")
        try:
            locks = canonicalize_visible_text_locks(raw_locks)
            digest = visible_text_locks_sha256(locks)
            if str(approval.get("visible_text_locks_sha256") or "").lower() != digest:
                raise VisibleTextContractError("visible text lock SHA does not match")
            if source_content_timeline is None:
                raise VisibleTextContractError("source content timeline is unavailable")
            locks = validate_visible_text_locks(locks, timeline=source_content_timeline)
        except VisibleTextContractError as exc:
            raise ProductionPortsError("storyboard approved visible text locks are invalid") from exc
        return locks, digest

    @classmethod
    def _uploaded_audio_evidence(
        cls,
        context: Any,
        artifacts: Sequence[Mapping[str, str]],
        *,
        uploaded_audio_sha256: str | None,
    ) -> dict[str, Any] | None:
        """Expose a classified upload to script review, never a late prompt pass."""

        if uploaded_audio_sha256 is None:
            return None
        rows = [item for item in artifacts if item["kind"] == "uploaded_audio_classification"]
        if len(rows) != 1:
            raise ProductionPortsError(
                "uploaded audio requires exactly one immutable song/non-song classification before script drafting"
            )
        artifact = rows[0]
        value = cls._materialized_json(
            context,
        ...
        try:
            contract = validate_uploaded_audio_contract(value, audio_sha256=uploaded_audio_sha256)
        except UploadedAudioContractError as exc:
            raise ProductionPortsError("uploaded audio classification is invalid or ambiguous") from exc
        lyrics = [dict(item) for item in contract["lyrics"] if isinstance(item, Mapping)]
        kind = str(contract["kind"])
        instruction = (
            "Show these exact uploaded-song lyrics in the editable script for user confirmation; assign each sung line to an explicitly confirmed on-camera performer."
            if kind == "song"
            else "This is a non-song audio replacement: do not create lyrics, singer roles, or lip-sync instructions."
        )
        return {
            "artifact_sha256": artifact["sha256"],
            "kind": kind,
    ...
    def _revision_evidence(
        ...
        *,
        ...
        if not isinstance(manifest, Mapping):
            raise ProductionPortsError("current job slot manifest is required")
        requested_language = manifest.get("output_language")
        if requested_language is None:
            language = "und"
        elif isinstance(requested_language, str) and requested_language in _OUTPUT_LANGUAGES:
            language = requested_language
        else:
            raise ProductionPortsError("current job output_language is unsupported")
        ...
        if isinstance(music, Mapping):
            music_hashes = music.get("sha256")
            if (
                music.get("extension_id") != "input_contract_v2.background_music"
                or not isinstance(music_hashes, list)
                or len(music_hashes) != 1
                or _SHA256.fullmatch(str(music_hashes[0]).lower()) is None
            ):
                raise ProductionPortsError("current job background music evidence is invalid")
            uploaded_audio_sha256 = str(music_hashes[0]).lower()
        ...
        if kind == "storyboard":
            ...
            visible_text_locks, visible_text_locks_digest = cls._approved_visible_text_locks(
                context,
                source_content_timeline=source_content_timeline,
                script_sha256=parent_revision_sha256,
            )
        else:
            visible_text_locks, visible_text_locks_digest = cls._default_visible_text_locks(
                source_content_timeline
            )
        uploaded_audio = cls._uploaded_audio_evidence(
            context,
            artifacts,
        ...
        if kind == "script" and uploaded_audio is not None:
            base["uploaded_audio"] = uploaded_audio
        if storyboard_performance is not None:
            base["approved_performance_lines"] = storyboard_performance
        if kind == "storyboard":
            base["approved_visible_text_locks"] = visible_text_locks
            base["approved_visible_text_locks_sha256"] = visible_text_locks_digest
        return {**base, "request_evidence_sha256": _sha256(base)}

    @classmethod
    def _language_only_instruction(cls, *, kind: str, evidence: Mapping[str, Any]) -> str:
        if evidence.get("output_language") == "und":
            return (
                "No output language was selected. Preserve the source language, exact meaning, speaker assignment, "
                "delivery cadence, and visible text language. Do not translate or choose a default language."
            )
    ...
    def _validate_response(self, response: Mapping[str, Any], *, context: Any, kind: str) -> Mapping[str, Any]:
        ...
        if str(value.get("visible_text_locks_sha256") or "").lower() != evidence["visible_text_locks_sha256"]:
            raise ProductionPortsError("GPT creative response visible text lock SHA does not match the current job")
        cuts = value.get("cuts")
        ...
        raw_locks = evidence.get("visible_text_locks")
        if not isinstance(raw_locks, list):
            raise ProductionPortsError("current job visible text locks are invalid")
        try:
            visible_text_locks = canonicalize_visible_text_locks(raw_locks)
            if visible_text_locks_sha256(visible_text_locks) != evidence["visible_text_locks_sha256"]:
                raise VisibleTextContractError("visible text lock SHA does not match")
        except VisibleTextContractError as exc:
            raise ProductionPortsError("current job visible text locks are invalid") from exc
        enriched_cuts: list[dict[str, Any]] = []
        for cut in cuts:
            ...
            enriched_cuts.append(
                {
                    **dict(cut),
                    "visible_text_locks": [
                        dict(lock) for lock in visible_text_locks if cut_id in lock["cut_ids"]
                    ],
                }
            )
        value = {
            **dict(value),
            "cuts": enriched_cuts,
            "visible_text_locks": [dict(lock) for lock in visible_text_locks],
    ...
    def _request_structured(
        ...
        *,
        ...
        cached = self._response_cache.get(request_sha256)
        if cached is not None:
            return json.loads(json.dumps(cached, ensure_ascii=False, sort_keys=True))
        try:
        ...
        self._response_cache[request_sha256] = json.loads(
...
class _CreativeRevisionStage:
    ...
    def run(self, *, context: Any, input_artifacts: Sequence[Mapping[str, Any]]) -> Mapping[str, Any]:
        ...
        visible_text_locks = draft["value"].get("visible_text_locks")
        visible_text_locks_sha = draft["value"].get("visible_text_locks_sha256")
        if not isinstance(visible_text_locks, list) or not isinstance(visible_text_locks_sha, str):
            raise ProductionPortsError("creative revision visible text locks are invalid")
        try:
            canonical_visible_text_locks = canonicalize_visible_text_locks(visible_text_locks)
            if visible_text_locks_sha256(canonical_visible_text_locks) != visible_text_locks_sha:
                raise VisibleTextContractError("visible text lock SHA does not match")
        except VisibleTextContractError as exc:
            raise ProductionPortsError("creative revision visible text locks are invalid") from exc
        payload = {
            "schema_version": _REVISION_SCHEMA_VERSION,
            "kind": self._kind,
            "revision": revision,
            ...
            "visible_text_locks_sha256": visible_text_locks_sha,
            "gpt_receipt": draft["receipt"],
        }
        if performance_candidates is not None:
            payload["performance_line_candidates"] = performance_candidates
        if self._kind == "script" and isinstance(evidence.get("uploaded_audio"), Mapping):
            payload["uploaded_audio"] = dict(evidence["uploaded_audio"])
        if self._kind == "storyboard":
            payload["approved_visible_text_locks_sha256"] = visible_text_locks_sha
        data = _canonical_json(payload)
        sha256 = hashlib.sha256(data).hexdigest()
        user_script_bytes: bytes | None = None
        if self._kind == "script":
            try:
                user_script_bytes = render_user_script_markdown(payload, canonical_visible_text_locks).encode("utf-8")
            except UserScriptDocumentError as exc:
                raise ProductionPortsError("creative revision user script document is invalid") from exc
        ...
        user_script_artifact: Mapping[str, Any] | None = None
        if user_script_bytes is not None:
            user_script_sha256 = hashlib.sha256(user_script_bytes).hexdigest()
            try:
                published_document = publisher(
                ...
                raise ProductionPortsError("user script document could not be published") from exc
            if not isinstance(published_document, Mapping):
                raise ProductionPortsError("user script document publication returned an invalid artifact")
            if self._planner._sha256_field(published_document.get("sha256"), "published user script SHA") != user_script_sha256:
                raise ProductionPortsError("published user script SHA does not match the rendered document")
            user_script_artifact = dict(published_document)
            published_artifacts.append(dict(published_document))
        parent_script_sha256 = evidence["parent_revision_sha256"] if self._kind == "storyboard" else None
        pending = getattr(getattr(context, "snapshot", None), "pending_review_request", None)
        revision_request = None
        if isinstance(pending, Mapping) and pending.get("kind") == self._kind:
            raw_request = pending.get("request")
            if not isinstance(raw_request, Mapping):
                raise ProductionPortsError("pending user revision request is invalid")
            try:
                revision_request = RevisionRequest(**dict(raw_request))
            except TypeError as exc:
                raise ProductionPortsError("pending user revision request is invalid") from exc
        manifest = RevisionManifest(
            ...
            changed_cut_ids=tuple(revision_request.changed_cut_ids) if revision_request is not None else (),
            output_language=evidence["output_language"],
        )
        result = {
        ...
        if user_script_artifact is not None:
            result["user_script_markdown"] = user_script_artifact
        return result
...
class RunningHubSeedanceMediaUploader:
    """Standard-model upload boundary authenticated only with the Seedance key."""

    _REQUEST_TIMEOUT_SECONDS = 120.0

    def __init__(self, config: ProductionEnvironment) -> None:
        self.config = config

    def upload_media(self, path: Path) -> str:
        return _upload_binary(
            url=self.config.runninghub_seedance_upload_url,
            headers={
                "Authorization": (
                    f"Bearer {_read_environment_secret(self.config.runninghub_seedance_api_key_env)}"
                )
            },
            path=Path(path),
            timeout_seconds=self._REQUEST_TIMEOUT_SECONDS,
...
class RunningHubSeedanceProvider:
    ...
    def create_video(self, request: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        _validate_server_video_reference_binding(request)
        _validate_server_audio_reference_binding(
            request, capability_secret_env=self.config.capability_secret_env
        )
        authorization_preflight = _server_provider_request_authorization_preflight(request)
        return self._create(
            operation="video",
            url=self.config.runninghub_seedance_create_url,
            payload=dict(request),
            api_key_env=self.config.runninghub_seedance_api_key_env,
            preflight=authorization_preflight,
        )
    ...
    def _create(
        ...
        *,
        ...
        # This is intentionally immediately adjacent to the paid transport.
        # A valid-looking sidecar is not authority unless the injected, server
        # verifier rechecks the current job/artifact/attempt state at this point.
...
def _validate_server_video_reference_binding(payload: Mapping[str, Any]) -> None:
    """Require an internal immutable binding for the optional source segment."""

    video_urls = payload.get("videoUrls")
    if not isinstance(video_urls, list):
        raise ProductionPortsError("RunningHub Seedance videoUrls must be an array")
    binding = getattr(payload, "video_reference_binding", None)
    if not video_urls and binding is None:
        return
    if not video_urls:
        raise ProductionPortsError("RunningHub Seedance video reference binding requires videoUrls[0]")
    if not isinstance(binding, Mapping):
        raise ProductionPortsError(
            "RunningHub Seedance video reference binding is required for a source segment"
        )
    try:
        validate_video_reference_binding(payload, binding)
        validate_final_reference_lineage(
            payload, getattr(payload, "final_reference_lineage", None)
        )
    except RunningHubStandardPayloadError as error:
        raise ProductionPortsError(f"RunningHub Seedance {error}") from error


def _validate_server_audio_reference_binding(
    payload: Mapping[str, Any], *, capability_secret_env: str = "USFR_CAPABILITY_SECRET"
) -> None:
    """Require an internal immutable binding for the optional music slice."""

    audio_urls = payload.get("audioUrls")
    if not isinstance(audio_urls, list):
        raise ProductionPortsError("RunningHub Seedance audioUrls must be an array")
    binding = getattr(payload, "audio_reference_binding", None)
    if not audio_urls and binding is None:
        return
    if not audio_urls:
        raise ProductionPortsError("RunningHub Seedance audio reference binding requires audioUrls[0]")
    if not isinstance(binding, Mapping):
        raise ProductionPortsError(
            "RunningHub Seedance audio reference binding is required for a duration-bounded music slice"
        )
    try:
        validate_audio_reference_binding(payload, binding)
        validate_provider_audit_proof(
            payload,
            getattr(payload, "video_reference_binding", None),
            binding,
            getattr(payload, "provider_audit_proof", None),
            secret=_read_environment_secret(capability_secret_env),
            audio_reference_artifact_receipt=getattr(
                payload, "audio_reference_artifact_receipt", None
            ),
        )
    except RunningHubStandardPayloadError as error:
        raise ProductionPortsError(f"RunningHub Seedance {error}") from error
...
def _server_provider_request_authorization_preflight(request: Mapping[str, Any]) -> Callable[[], None] | None:
    """Return the private verifier for one audited provider attempt.

    The public fixed-B payload deliberately cannot carry this object.  It is
    attached only by ``SubmitProviderVideoStage`` after it has read the durable
    audit artifact and created the Provider attempt.  Direct/legacy provider
    tests without source/audio/sidecar evidence retain the compatibility path.
    """

    requires_authorization = bool(request.get("videoUrls") or request.get("audioUrls")) or hasattr(
        request, "final_reference_lineage"
    )
    if not requires_authorization:
        return None
    from .audio_provider_authorization import ServerAudioAuthorizationVerifier

    authorization = getattr(request, "audio_provider_authorization", None)
    verifier = getattr(request, "server_audio_authorization_verifier", None)
    if not isinstance(authorization, Mapping) or not isinstance(verifier, ServerAudioAuthorizationVerifier):
        raise ProductionPortsError("RunningHub Seedance requires a server-issued provider request authorization")

    def verify() -> None:
        try:
            verifier.verify_and_consume(
                payload=dict(request),
                video_reference_binding=getattr(request, "video_reference_binding", None),
                final_reference_lineage=getattr(request, "final_reference_lineage", None),
                audio_reference_binding=getattr(request, "audio_reference_binding", None),
                audio_reference_artifact_receipt=getattr(request, "audio_reference_artifact_receipt", None),
                authorization=authorization,
            )
        except Exception as error:
            # Never classify a failed preflight as an ambiguous paid create:
            # no HTTP has happened when this is raised.
            raise ProductionPortsError(f"RunningHub Seedance audio provider authorization rejected: {error}") from error
