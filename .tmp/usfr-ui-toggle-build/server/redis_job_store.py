...
local stored_dedupe = redis.call('HGET', KEYS[3], '@meta:' .. stage .. ':dedupe')
local lease = redis.call('GET', KEYS[2])
if status ~= 'CLAIMED' or stored_owner ~= ARGV[6] or stored_dedupe ~= ARGV[5] then
    return {'CONFLICT'}
end
if not lease or lease ~= ARGV[12] then
    return {'LEASE_LOST'}
end
if current_version ~= tonumber(ARGV[2]) or tonumber(ARGV[3]) ~= current_version + 1 then
    return {'STALE', tostring(current_version)}
end
redis.call('HSET', KEYS[3], stage, ARGV[7],
    '@meta:' .. stage .. ':status', 'FAILED')
redis.call('DEL', KEYS[2])
persist_snapshot(KEYS[1], ARGV[1], tonumber(ARGV[3]), ARGV[10], ARGV[11])
touch_keys(tonumber(ARGV[8]), KEYS[4], KEYS[3], ARGV[9], {KEYS[1], KEYS[3], KEYS[5], KEYS[7], KEYS[8], KEYS[9]})
refresh_provider_due(KEYS[5], KEYS[6], ARGV[9], tonumber(ARGV[8]))
return {'OK', ARGV[7]}
"""
...
class RedisEphemeralJobStore(EphemeralJobStore):
    ...
    def create_job(
        ...
        *,
        ...
        if not isinstance(initial_state, str) or not initial_state.strip():
            raise ReplicationError("INVALID_INPUT", "initial_state must be a non-empty string")
    ...
    def cas_transition(
        ...
        *,
        ...
        for field, value in update_values.items():
            ...
            if field == "pending_review_request" and value is not None and not isinstance(value, Mapping):
                raise ReplicationError("INVALID_INPUT", "pending_review_request must be a mapping or null")
            if field == "public_error":
                try:
                    from .public_errors import validate_public_error

                    value = validate_public_error(value)
                except ValueError as exc:
                    raise ReplicationError("INVALID_INPUT", str(exc)) from exc
    ...
    def append_revision(
        ...
        *,
        ...
        pending = current.pending_review_request
        if isinstance(pending, Mapping) and pending.get("kind") == kind:
            payload["pending_review_request"] = None
        candidate = JobSnapshot.from_dict(payload)
        keys = [
            self._job_key(job_id),
            revisions_key,
    ...
    def fail_stage(
        ...
        *,
        ...
        if not isinstance(terminal, bool):
            raise ReplicationError("INVALID_INPUT", "terminal must be boolean")
        try:
            from .public_errors import validate_public_error

            normalized_error = validate_public_error(public_error)
        except ValueError as exc:
            raise ReplicationError("INVALID_INPUT", str(exc)) from exc
        if terminal != (normalized_error is not None):
            raise ReplicationError("INVALID_INPUT", "terminal stage failure requires exactly one public_error")
        current = self._require_snapshot(job_id)
        existing_raw = self.redis.hget(self._stages_key(job_id), stage)
        if not existing_raw:
            raise StateConflictError("stage checkpoint does not exist")
        ...
        if terminal:
            ...
            payload["public_error"] = normalized_error
        candidate = JobSnapshot.from_dict(payload)
        ...
        checkpoint_json = _canonical_json({**checkpoint.__dict__, "output_artifact_ids": []})
        script_revision, storyboard_revision = _snapshot_meta(candidate)
        keys = [
            self._job_key(job_id),
            self._lease_key(job_id, stage),
        ...
        result = self._eval(
            ...
            [
                ...
                script_revision,
                storyboard_revision,
                _canonical_json({"owner": owner, "dedupe_key": dedupe_key, "attempt": int(existing.attempt)}),
            ],
        )
        status = self._status(result)
        if status == "GONE":
        ...
        if status in {"CONFLICT", "LEASE_LOST", "STALE", "STAGE_GONE"}:
            raise StateConflictError("stage lease or checkpoint fence rejected failure")
        if status != "OK":
            raise ReplicationError("STORE_ERROR", "stage failure commit failed")
        return checkpoint
