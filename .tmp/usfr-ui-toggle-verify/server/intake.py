...
_AUDIO_MIME = {"audio/aac", "audio/flac", "audio/mp4", "audio/mpeg", "audio/ogg", "audio/wav"}
...
def _validate_object_ref(value: Mapping[str, Any], *, slot_id: str, kind: str) -> tuple[str, str, dict[str, Any]]:
    ...
    allowed_mime = _VIDEO_MIME if kind == "video" else _IMAGE_MIME if kind == "image" else _AUDIO_MIME
...
def bind_uploaded_slots(
    ...
    *,
    ...
    has_object_completion = any(_object_ref(item) for raw in slot_objects.values() for item in _values(raw)) or any(
        _object_ref(item) for item in _values(background_music)
    )
    ...
    if not has_object_completion:
        ...
        try:
            ...
            normalized_music = background_music
            if isinstance(background_music, Mapping):
                normalized_music = background_music.get("path", background_music.get("url", background_music))
            manifest = validate_slots(
                normalized,
                background_music=normalized_music,
    ...
    from scripts.bind_input_slots import (
        OPTIONAL_SLOTS,
        SLOT_ORDER,
        _normalize_ui_rebuild_enabled,
        _route_defaults,
        _SLOT_KIND,
        _SLOT_ROLE,
        _url_value,
    ...
    music_extension: dict[str, Any] | None = None
    music_values = _values(background_music)
    if len(music_values) > 1:
        raise ReplicationError("INPUT_SLOT_INVALID", "background_music: only one value is allowed", category="input", user_action_required=True, http_status=400)
    if music_values:
        item = music_values[0]
        if not isinstance(item, Mapping):
            raise ReplicationError("INPUT_SLOT_INVALID", "background_music requires an immutable upload-completion object", category="input", user_action_required=True, http_status=400)
        try:
            key, digest, metadata = _validate_object_ref(item, slot_id="background_music", kind="audio")
        except ValueError as exc:
            raise ReplicationError("INPUT_SLOT_INVALID", str(exc), category="input", user_action_required=True, http_status=400) from exc
        if object_store is None or not upload_scope:
            raise ReplicationError("INPUT_SLOT_INVALID", "background_music requires a configured object store and upload_scope", category="input", user_action_required=True, http_status=400)
        if not key.startswith(f"uploads/{upload_scope}/"):
            raise ReplicationError("INPUT_SLOT_INVALID", "background_music: object_key is outside the upload scope", category="input", user_action_required=True, http_status=400)
        try:
            observed = object_store.head(key)
        except Exception as exc:
            raise ReplicationError("OBJECT_STORE_UNAVAILABLE", "background_music object-store metadata could not be verified", category="storage", retryable=True, http_status=503) from exc
        observed_key = str(observed.get("object_key") or observed.get("object_uri") or observed.get("uri") or "").strip() if isinstance(observed, Mapping) else ""
        if (
            observed_key != key
            or str(observed.get("sha256") or "").lower() != digest
            or int(observed.get("size_bytes", -1)) != metadata["size_bytes"]
            or str(observed.get("content_type") or "").lower() != metadata["content_type"]
            or str(observed.get("status") or "completed").lower() != "completed"
        ):
            raise ReplicationError("ARTIFACT_METADATA_MISMATCH", "background_music upload completion differs from the stored object", category="artifact", user_action_required=True, http_status=422)
        metadata["store_verified"] = True
        music_extension = {
    ...
    if optional_count < 1 and music_extension is None and not allow_language_only:
    ...
    language_only = bool(allow_language_only) and music_extension is None
    manifest = {
        ...
        "review_route": "route_2" if language_only else None,
    }
    manifest["routes"]["background_music"] = "seedance_audio_reference" if music_extension is not None else "none"
    manifest["extensions"] = {"ui_rebuild_enabled": configured_ui_rebuild_enabled}
    if music_extension is not None:
        manifest["extensions"]["background_music"] = music_extension
    if output_language is not None:
        manifest["output_language"] = str(output_language).strip().lower()
    return manifest
