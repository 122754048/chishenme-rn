...
class EvidenceBoundHttpUiRenderer(_EvidenceBoundHttpBackend):
    ...
    def _motion_reference(
        ...
        *,
        ...
        interaction = render_contract.get("source_ui_interaction_contract")
        if not isinstance(interaction, Mapping):
            return None
        source_fps = interaction.get("source_fps")
        roi = interaction.get("ui_roi")
        if not isinstance(source_fps, Mapping) or not isinstance(roi, Mapping):
            return None
        try:
            start_us = int(interaction["source_window_us"]["start"])
            end_us = int(interaction["source_window_us"]["end_exclusive"])
            start_frame = int(interaction["frame_window"]["start"])
            end_frame = int(interaction["frame_window"]["end_exclusive"])
            fps_num = int(source_fps["num"])
            fps_den = int(source_fps["den"])
        except (KeyError, TypeError, ValueError) as exc:
            raise VisionBackendUnavailable(
                "source UI interaction contract cannot materialize a motion reference",
                retryable=False,
            ) from exc
        expected_frames = end_frame - start_frame
        if start_us < 0 or end_us <= start_us or expected_frames <= 0 or fps_num <= 0 or fps_den <= 0:
            raise VisionBackendUnavailable(
                "source UI interaction contract has an invalid motion window",
                retryable=False,
            )
        materialize = getattr(context, "materialize_slot", None)
        if not callable(materialize):
            raise VisionBackendUnavailable(
                "UI motion reference requires context.materialize_slot('source_video')",
                retryable=False,
            )
        ffmpeg = shutil.which("ffmpeg")
        ffprobe = self.ffprobe_bin or shutil.which("ffprobe")
        if not ffmpeg or not ffprobe:
            raise VisionBackendUnavailable(
                "FFmpeg and FFprobe are required to materialize the source UI interval",
                retryable=False,
            )
        work_dir = Path(getattr(context, "work_dir", tempfile.gettempdir()))
        work_dir.mkdir(parents=True, exist_ok=True)
        try:
            materialized = materialize("source_video")
        except TypeError:
            materialized = materialize(slot_id="source_video")
        with materialized as source_media, tempfile.TemporaryDirectory(
            prefix="usfr-ui-motion-", dir=str(work_dir)
        ) as raw:
            source_path = Path(source_media.path)
            output_path = Path(raw) / "motion-reference.mp4"
            command = [
                str(ffmpeg),
                "-y",
                "-loglevel",
                ...
                str(expected_frames),
                "-vf",
                f"fps={fps_num}/{fps_den},setpts=PTS-STARTPTS",
                "-c:v",
                "libx264",
                "-preset",
                "veryfast",
            ...
            try:
                ...
                raise VisionBackendUnavailable(
                    "source UI motion interval could not be materialized",
                    retryable=False,
                ) from exc
            ...
            try:
                actual_frames = int(probe.stdout.decode("utf-8").strip()) if probe.returncode == 0 else 0
            except (UnicodeDecodeError, ValueError):
                actual_frames = 0
            if actual_frames != expected_frames:
                raise VisionBackendUnavailable(
                    "source UI motion interval frame count does not match the frozen contract",
                    details={"expected_frames": expected_frames, "actual_frames": actual_frames},
                    retryable=False,
                )
            ...
            return {
                ...
                "source_ui_interaction_contract": dict(interaction),
            }
    ...
    def __call__(
        ...
        *,
        ...
        motion_reference = self._motion_reference(context=context, render_contract=render_contract)
        if motion_reference is not None:
            payload["motion_reference"] = motion_reference
        response, request_sha256, response_sha256 = self._post(payload)
