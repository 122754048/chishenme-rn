...
from .analysis_scope import build_analysis_scope, validate_execution_scope, validate_tool_call
...
class EphemeralWorkerManager:
    ...
    @staticmethod
    def _validate_tool_execution_receipts(
        context: EphemeralStageContext,
        result: Mapping[str, Any],
    ) -> None:
        receipts = result.get("tool_execution_receipts")
        if receipts is None:
            return
        if not isinstance(receipts, Sequence) or isinstance(receipts, (str, bytes, bytearray)):
            raise ReplicationError(
                "CONTRACT_INVALID",
                "tool_execution_receipts must be a sequence",
                category="analysis",
                http_status=422,
            )
        for receipt in receipts:
            if not isinstance(receipt, Mapping):
                raise ReplicationError(
                    "CONTRACT_INVALID",
                    "tool execution receipt must be an object",
                    category="analysis",
                    http_status=422,
                )
            ...
            if not tool_name:
                raise ReplicationError(
                    "CONTRACT_INVALID",
                    "tool execution receipt has no tool name",
                    category="analysis",
                    http_status=422,
                )
            context.authorize_tool(
                tool_name,
                promotion_receipt=receipt.get("promotion_receipt")
                if isinstance(receipt.get("promotion_receipt"), Mapping)
                else None,
            )
    ...
    def _hydrate_stage_outputs(self, context: EphemeralStageContext) -> None:
        ...
        for stage in self._known_stages():
            ...
            if artifact_id is None:
                continue
            pending.append((stage, artifact_id, artifacts[artifact_id]["metadata"]))

        def _fetch(item: tuple[str, str, Mapping[str, Any]]) -> Any:
            _stage, artifact_id, _metadata = item
            with context.materialize_artifact("stage_output", artifact_id=artifact_id) as media:
                return json.loads(Path(media.path).read_text(encoding="utf-8"))

        # Every prior stage output is an independent, digest-verified read.
        # Fetching them concurrently removes the serial per-stage object-store
        # round trip without changing which artifacts are read or how each one
        # is validated below.
        try:
            if len(pending) <= 1:
                payloads = [_fetch(item) for item in pending]
            else:
                with ThreadPoolExecutor(
                    max_workers=min(len(pending), 8),
                    thread_name_prefix="stage-output-hydrate",
                ) as executor:
                    payloads = list(executor.map(_fetch, pending))
        ...
        for (stage, artifact_id, metadata), payload in zip(pending, payloads):
            if (
        ...
        if not isinstance(route_output, Mapping):
            return
        execution_scope = route_output.get("execution_scope")
        if execution_scope is not None:
            if not isinstance(execution_scope, Mapping):
                raise ReplicationError("CONTRACT_INVALID", "route_regions execution scope is invalid")
            context.analysis_scope = validate_execution_scope(
                execution_scope,
                pre_route_scope=context.analysis_scope,
            )
    ...
    def _apply_result_authority(
        ...
        *,
        ...
        if context.stage == "import_sources":
            manifest = result.get("slots_manifest")
            if not isinstance(manifest, Mapping):
                raise ReplicationError("CONTRACT_INVALID", "import_sources must return slots_manifest")
            ...
            self.job_store.cas_transition(
                ...
                command="complete_public_import",
                updates={"slots_manifest": dict(manifest), "state": "ANALYZING"},
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(prefix=f"usfr-{message.job_id[:8]}-") as temporary:
            ...
            try:
                ...
                self._validate_tool_execution_receipts(context, output)
