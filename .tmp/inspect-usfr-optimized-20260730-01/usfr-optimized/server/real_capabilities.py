...
from .ui_interaction_contract import (
...
class FfmpegDynamicsAnalyzer:
    ...
    def analyze(self, *, context: Any, input_artifacts: list[Mapping[str, Any]]) -> Mapping[str, Any]:
        authorize = getattr(context, "authorize_tool", None)
        if callable(authorize):
            authorize("structural_timeline")
        tool_execution_receipts: list[dict[str, str]] = [
            {"tool": "structural_timeline", "stage": "analyze_dynamics", "status": "executed"}
        ]
        with _materialize(context, "source_video") as media:
            ...
            if semantic_required and self.semantic_analyzer is not None:
                if callable(authorize):
                    authorize("semantic_vlm")
                tool_execution_receipts.append(
                    {"tool": "semantic_vlm", "stage": "analyze_dynamics", "status": "executed"}
                )
        ...
        with _materialize(context, "source_video") as media:
            ...
            return {
                ...
                "tool_execution_receipts": [
                    {"tool": "source_asr", "stage": str(getattr(context, "stage", "transcribe_audio")), "status": "executed"}
                ],
                "audio_contract": {
    ...
    def run(
        ...
        *,
        ...
        return {
            "status": "ready",
            "tool_execution_receipts": [
                {"tool": "app_store_evidence", "stage": "parse_app_store_evidence", "status": "executed"}
            ],
...
class DeterministicUiRenderer:
    ...
    @staticmethod
    def _source_ui_interaction_contract(context: Any) -> Mapping[str, Any] | None:
        """Return the one frozen UI-only motion contract for this render.

        This is deliberately a metadata-only boundary.  It does not inspect a
        video or run OCR: the UI Cut was already identified by dynamics
        routing, and only the deterministic UI renderer may consume it.
        """

        contracts: list[Mapping[str, Any]] = []
        for region in getattr(context, "timeline_regions", ()) or ():
            if not isinstance(region, Mapping):
                continue
            region_type = str(region.get("region_type") or region.get("kind") or "").casefold()
            if region_type not in {"generated_ui_demo", "generated_ui"}:
                continue
            raw_contract = region.get("source_ui_interaction_contract")
            if raw_contract is None:
                continue
            try:
                contract = validate_source_ui_interaction_contract(raw_contract)
            except UiInteractionContractError as exc:
                raise CapabilityUnavailable(
                    "source UI interaction contract is invalid",
                    details={"region_id": str(region.get("region_id") or ""), "reason": str(exc)},
                ) from exc
            region_id = str(region.get("region_id") or "").strip()
            if region_id and contract["region_id"] != region_id:
                raise CapabilityUnavailable(
                    "source UI interaction contract region_id does not match the generated UI region"
                )
            declared_digest = str(region.get("source_ui_interaction_contract_sha256") or "").strip().lower()
            if declared_digest and declared_digest != _canonical_sha256(contract):
                raise CapabilityUnavailable(
                    "source UI interaction contract SHA-256 does not match the frozen region contract"
                )
            try:
                source_start = region.get("source_start_us")
                source_end = region.get("source_end_us")
                if source_start is not None and int(contract["source_window_us"]["start"]) != int(source_start):
                    raise CapabilityUnavailable(
                        "source UI interaction contract start does not match the generated UI region"
                    )
                if source_end is not None and int(contract["source_window_us"]["end_exclusive"]) != int(source_end):
                    raise CapabilityUnavailable(
                        "source UI interaction contract end does not match the generated UI region"
                    )
            except (KeyError, TypeError, ValueError) as exc:
                raise CapabilityUnavailable("source UI interaction contract timing is invalid") from exc
            contracts.append(contract)
        if len(contracts) > 1:
            raise CapabilityUnavailable(
                "generated UI renderer requires one frame-locked source UI interaction contract per render"
            )
        return contracts[0] if contracts else None

    @contextmanager
    def _ui_source(self, context: Any):
        interaction_contract = self._source_ui_interaction_contract(context)
        ...
        if interaction_contract is not None:
            for slot_id in ("new_product_image", "new_model_image"):
                if slot_id not in present:
                    continue
                with _materialize(context, slot_id) as media:
                    metadata = present[slot_id].get("metadata") or []
                    record = metadata[0] if metadata and isinstance(metadata[0], Mapping) else {}
                    yield Path(media.path), slot_id, dict(record)
                return
        ...
        *,
        ...
        if interaction_contract is not None:
            if not isinstance(target_contract, Mapping):
                raise CapabilityUnavailable(
                    "frame-locked source UI interaction requires an immutable target UI render contract"
                )
            target_contract = deepcopy(dict(target_contract))
            existing_contract = target_contract.get("source_ui_interaction_contract")
            if isinstance(existing_contract, Mapping) and _canonical_sha256(existing_contract) != _canonical_sha256(interaction_contract):
                raise CapabilityUnavailable(
                    "target UI render contract cannot replace the frozen source UI interaction contract"
                )
            target_contract["source_ui_interaction_contract"] = deepcopy(dict(interaction_contract))
            target_contract["source_ui_interaction_contract_sha256"] = _canonical_sha256(interaction_contract)
            target_contract["language"] = deepcopy(dict(interaction_contract["language"]))
            target_contract["text_encoding"] = deepcopy(dict(interaction_contract["text_encoding"]))
        ...
        sidecar_request_sha256 = str(
            (rendered.get("backend_evidence") or {}).get("request_sha256")
            if isinstance(rendered.get("backend_evidence"), Mapping)
            else ""
        ).lower()
        media_metadata = {
        ...
        if renderer_decision.get("backend") == "remotion_react_ui" and len(sidecar_request_sha256) == 64:
            media_metadata["ui_sidecar_request_sha256"] = sidecar_request_sha256
    ...
    def _extract_ui_frame(
        ...
        *,
        ...
        # One decode pass emits both the encoded PNG evidence and the decoded
        # RGB24 bytes.  Both outputs use the same encoders as the previous
        # two-process form, so the PNG bytes and the rawvideo digest are
        # unchanged; only the duplicate seek/decode is removed.
        ...
        if combined.returncode != 0 or not png_path.is_file() or png_path.stat().st_size <= 0:
            raise CapabilityUnavailable(
                f"generated UI frame extraction failed for state {state_id}",
                details={"stderr": combined.stderr.decode("utf-8", errors="replace")[-500:]},
            )
        if not combined.stdout:
            raise CapabilityUnavailable(
                f"generated UI decoded frame evidence failed for state {state_id}",
                details={"stderr": combined.stderr.decode("utf-8", errors="replace")[-500:]},
            )
        return png_path, _sha256_bytes(combined.stdout)
    ...
    def render_and_verify(self, *, context: Any, input_artifacts: list[Mapping[str, Any]]) -> Mapping[str, Any]:
        authorize = getattr(context, "authorize_tool", None)
        if callable(authorize):
            authorize("ui_rebuild")
            authorize("target_ui_ocr")
        tool_execution_receipts = [
            {"tool": "ui_rebuild", "stage": str(getattr(context, "stage", "resolve_ui_evidence")), "status": "executed"},
            {"tool": "target_ui_ocr", "stage": str(getattr(context, "stage", "resolve_ui_evidence")), "status": "executed"},
        ]
        with self._ui_source(context) as (source, slot, source_metadata):
            if _active_high_fidelity(context) and _has_generated_ui_region(context):
                active_result = dict(self._render_active_generated_ui_video(
                    context=context,
                    source=source,
                    source_metadata=source_metadata,
                    slot=slot,
                ))
                active_result["tool_execution_receipts"] = tool_execution_receipts
...
class FfmpegQcEngine:
    ...
    def run(self, *, context: Any, input_artifacts: list[Mapping[str, Any]]) -> Mapping[str, Any]:
        ...
        with manager as media:
            ...
            black_filter = f"blackdetect=d={minimum_black:.6f}:pix_th=0.1:pic_th=1.0"
            freeze_filter = f"setpts=PTS-STARTPTS,freezedetect=n=-60dB:d={minimum_freeze:.6f}"
            # Both detectors consume the same decoded and downscaled frames and
            # write independent, separately parsed stderr records.  Splitting a
            # single decode keeps every filter expression, threshold, and log
            # line identical while removing one full-file decode pass.  A
            # deployment whose FFmpeg rejects the combined graph falls back to
            # the original two-pass form, so the QC verdict never changes.
            combined_scan = _run([
                ffmpeg,
                "-hide_banner",
                "-nostats",
                ...
                "-filter_complex",
                (
                    f"[0:v]scale=160:-2,split=2[qcblack][qcfreeze];"
                    f"[qcblack]{black_filter}[qcblackout];"
                    f"[qcfreeze]{freeze_filter}[qcfreezeout]"
                ),
                "-map", "[qcblackout]", "-an", "-f", "null", "-",
                "-map", "[qcfreezeout]", "-an", "-f", "null", "-",
            ])
            if combined_scan.returncode == 0:
                black_stderr = combined_scan.stderr
                freeze_stderr = combined_scan.stderr
            else:
                black_stderr = _run([
                    ffmpeg,
                    "-hide_banner",
                ...
                freeze_stderr = _run([
                    ...
                    str(path),
                    "-vf",
                    f"scale=160:-2,{freeze_filter}",
                    "-an",
                    "-f",
                    "null",
                    "-",
                ]).stderr
            black_intervals = _parse_black_detect_intervals(black_stderr)
            ...
            execution_scope = getattr(context, "analysis_scope", None)
            route_family = (
                str(execution_scope.get("route_family") or "unknown")
                if isinstance(execution_scope, Mapping)
                else "unknown"
            )
            qc_factor_scores: Mapping[str, Any] = {}
            if isinstance(weighted_input, Mapping) and isinstance(
                weighted_input.get("factor_scores"), Mapping
            ):
                qc_factor_scores = weighted_input["factor_scores"]
            qc_plan = build_qc_plan(
                route=route_family,
                hard_failures=hard_failures,
                factor_scores=qc_factor_scores,
            )
            report = {
