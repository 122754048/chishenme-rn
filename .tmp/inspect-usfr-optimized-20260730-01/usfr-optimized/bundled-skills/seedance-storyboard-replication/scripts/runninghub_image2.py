...
SCOPE_SCRIPT_DIR = SKILL_ROOT / "scripts"
...
def run_generation(
    *,
    ...
    covered_cut_ids: list[str] | None = None,
    ...
    if scope_receipt is None:
        raise ValueError("--scope-receipt is required before USFR image generation")
    receipt = json.loads(scope_receipt.read_text(encoding="utf-8-sig"))
    validate_scope_receipt_for_text(receipt, prompt)
    covered_cut_ids = list(covered_cut_ids or [])
    allowed_cut_ids = receipt.get("image_model_allowed_cut_ids")
    if not covered_cut_ids or not isinstance(allowed_cut_ids, list) or not set(covered_cut_ids).issubset(set(allowed_cut_ids)):
        raise ValueError("image generation may cover only receipt-authorized generated Cut IDs")
        ...
        try:
            ...
            request_record = {
                **payload,
                ...
                "timeline_scope_receipt": {"path": str(scope_receipt), "scope_sha256": receipt.get("scope_sha256")},
                "covered_cut_ids": covered_cut_ids,
                "reference_images": [str(path) for path in reference_images],
            }
    ...
    try:
        ...
        write_json(
            artifact_dir / f"{cut_id}.meta.json",
            {
                ...
                "timeline_scope_receipt_path": str(scope_receipt),
                "timeline_scope_receipt_sha256": receipt.get("scope_sha256"),
                "covered_cut_ids": covered_cut_ids,
...
def main() -> int:
    ...
    parser.add_argument("--covered-cut-id", action="append", required=True)
    ...
    run_generation(
        ...
        covered_cut_ids=args.covered_cut_id,
