...
def load_performance_benchmark_cases(path: str | Path) -> dict[str, str]:
    """Return the fixed same-case routes used by the performance release gate."""
    ...
    if not isinstance(payload, Mapping):
        raise ValueError("validation catalog must be an object")
    raw = payload.get("performance_benchmark_cases")
    required = {
        "standard",
        "compound_app_ui_audio",
        "deterministic_splice",
    }
    if not isinstance(raw, Mapping) or set(raw) != required:
        raise ValueError(
            "performance_benchmark_cases must define all route-first classes"
        )
    cases, _smoke_ids = load_catalog(path)
    known = {case["case_id"] for case in cases}
    result = {str(route): str(case_id) for route, case_id in raw.items()}
    if len(set(result.values())) != len(result):
        raise ValueError("performance benchmark case IDs must be unique")
    missing = set(result.values()) - known
    if missing:
        raise ValueError(
            f"performance benchmark cases are missing: {sorted(missing)}"
        )
    return result
